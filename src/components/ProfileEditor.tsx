...
interface ProfileEditorProps {
  ...
  /** Render only one logical section. Defaults to 'all' (legacy single-card layout). */
  section?: 'all' | 'avatar' | 'info' | 'appearance';
  embedded?: boolean;
}

const ProfileEditor = ({ profile, onProfileChange, onAutoSave, section = 'all', embedded = false }: ProfileEditorProps) => {
    ...
    try {
      ...
      if (publicUrl) {
        ...
        toast({
          ...
          description: saved ? "Tu foto se guardó automáticamente" : "Tu foto se subió, pero no se pudo guardar automáticamente",
          variant: saved ? "default" : "destructive",
        });
      }
    } catch {
      toast({ title: "Error", description: "Hubo un problema subiendo tu foto", variant: "destructive" });
    }
  };
  ...
  const handleCoverFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (publicUrl) {
        const nextProfile: ProfileData = { ...profile, coverType: 'image', coverImage: publicUrl };
        onProfileChange(nextProfile);
        const saved = onAutoSave ? await onAutoSave(nextProfile) : true;
        toast({
          title: saved ? "Portada guardada" : "Portada actualizada",
          description: saved ? "Tu portada se guardó automáticamente" : "Tu portada se subió, pero no se pudo guardar automáticamente",
          variant: saved ? "default" : "destructive",
        });
      }
    } catch {
      toast({ title: "Error", description: "Hubo un problema subiendo tu portada", variant: "destructive" });
    }
  };
  ...
  const AvatarSection = (
    <div className="flex items-center gap-4">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">JPG o PNG, máx. 5MB. Se guarda automáticamente.</p>
      </div>
    </div>
  );
  ...
  const AppearanceSection = (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <Label>Portada</Label>
        <div
          className="h-32 rounded-xl overflow-hidden relative group border"
          style={
        ...
        >
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
            <Button variant="secondary" size="sm" onClick={() => coverInputRef.current?.click()} className="text-xs" disabled={uploadingCover}>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={uploadingCover}
          >
            <Palette className="w-4 h-4 mr-2" /> Color sólido
          </Button>
        </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input id="name" value={profile.name} onChange={(e) => updateField("name", e.target.value)} placeholder="Tu nombre" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="title">Cargo / Título</Label>
            <Input id="title" value={profile.title} onChange={(e) => updateField("title", e.target.value)} placeholder="Ej: CEO, Diseñador, etc." />
          </div>
          <div className="space-y-2">
            <Label htmlFor="company">Empresa</Label>
            <Input id="company" value={profile.company} onChange={(e) => updateField("company", e.target.value)} placeholder="Nombre de tu empresa" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input id="email" type="email" value={profile.email} onChange={(e) => updateField("email", e.target.value)} placeholder="tu@email.com" />
          </div>
          <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Textarea id="bio" value={profile.bio} onChange={(e) => updateField("bio", e.target.value)} placeholder="Cuéntanos sobre ti..." rows={3} />
        </div>
      </div>
    ) : (
      <div className="space-y-4">
  ...
  if (section === 'avatar') content = AvatarSection;
  else if (section === 'info') content = InfoSection;
  else if (section === 'appearance') content = AppearanceSection;
  else content = (
    <div className="space-y-6">
      {AppearanceSection}
      {AvatarSection}
